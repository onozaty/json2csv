...
func run(arguments []string, output io.Writer) int {
	...
	flagSet := flag.NewFlagSet("json2csv", flag.ContinueOnError)

	flagSet.StringVarP(&jsonPath, "input", "i", "", "JSON input file path or directory or url")
	flagSet.StringVarP(&mappingPath, "mapping", "m", "", "JSON to CSV mapping file path or url")
	...
	flagSet.Usage = func() {
		fmt.Fprintf(output, "json2csv v%s (%s)\n\n", Version, Commit)
		fmt.Fprint(output, "Usage: json2csv [flags]\n\nFlags\n")
	...
	if jsonPath == "" || mappingPath == "" || csvPath == "" {
		flagSet.Usage()
		return NG
	}
	...
	jsonPaths, err := findJSON(jsonPath)
	if err != nil {
		fmt.Fprintln(output, err)
		return NG
	}
	...
	if err := convert(jsonPaths, mapping, csvFile); err != nil {
		fmt.Fprintln(output, err)
		return NG
	}
...
func convert(jsonPaths []string, mapping *Mapping, writer io.Writer) error {
	...
	for _, jsonPath := range jsonPaths {
		doc, err := parseJSON(jsonPath)
		if err != nil {
			return fmt.Errorf("%s is failed: %w", jsonPath, err)
		}
...
func convertOne(doc *jsonquery.Node, mapping *Mapping, csvWriter *customcsv.Writer) error {

	rows, err := jsonquery.QueryAll(doc, mapping.RowsPath)
	if err != nil {
...
func getValue(row *jsonquery.Node, valuePath string, useEvaluate bool) (string, error) {
	...
	if useEvaluate {
		...
		value := expr.Evaluate(jsonquery.CreateXPathNavigator(row))
		return fmt.Sprint(value), nil
	}

	// Nodeを返す場合
	value, err := jsonquery.Query(row, valuePath)
	if err != nil {
		return "", fmt.Errorf("xpath '%s' is failed: %w", valuePath, err)
	}
...
func parseJSON(path string) (*jsonquery.Node, error) {
	...
	return jsonquery.Parse(reader)
}

func findJSON(path string) ([]string, error) {
